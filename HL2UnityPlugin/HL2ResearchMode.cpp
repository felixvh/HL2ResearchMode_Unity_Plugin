...
namespace winrt::HL2UnityPlugin::implementation
{
    ...
    void HL2ResearchMode::DepthSensorLoop(HL2ResearchMode* pHL2ResearchMode)
    {
        ...
        try 
        {
            while (pHL2ResearchMode->m_depthSensorLoopStarted)
            {
                ...
                std::vector<float> poseArray{ rot.x, rot.y, rot.z,rot.w, pos.x,pos.y,pos.z};
                ...
                // save data
                {
                    ...
                    // save rigid node pose
                    if (!pHL2ResearchMode->m_rigidNodePose)
                    {
                        OutputDebugString(L"Create Space for rigid node pose...\n");
                        pHL2ResearchMode->m_rigidNodePose = new float[outBufferCount * 7];
                    }

                    memcpy(pHL2ResearchMode->m_rigidNodePose, poseArray.data(), poseArray.size() * sizeof(float));
    ...
    com_array<float> HL2ResearchMode::GetRigidNodePose()
    {
        std::lock_guard<std::mutex> l(mu); 
        com_array<float> rigidNodePose = com_array<float>(std::move_iterator(m_rigidNodePose), std::move_iterator(m_rigidNodePose + 7));

   
        return rigidNodePose;
